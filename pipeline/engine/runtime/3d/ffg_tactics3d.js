...
// (template) per-game music is pipeline-assigned (music_generator) + played by the shell
...
register3d("tactics3d", async (kernel, content) => {
  ...
  function showEnd() {
    if (endShown) return; endShown = true;
     // battle's over — fade the score (no music over the result screen)
  ...
  if (window.FFG && window.FFG.Shell) {
    shell = new window.FFG.Shell({
      ...
      music: (content.audio && content.audio.music) || content.music || null, // per-game unique track from the pipeline (music_generator)
