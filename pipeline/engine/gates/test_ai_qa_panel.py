...
cmd_v, prompt_v = A.build_command(V, CTX)
chk("vision argv is bare claude -p (prompt rides STDIN — WinError 206 guard)", cmd_v == ["claude", "-p"])
chk("vision prompt attaches @shot", "@/tmp/x.png" in prompt_v)
cmd_c, prompt_c = A.build_command(C, CTX)
chk("code argv bare + prompt has no @shot", cmd_c == ["claude", "-p"] and "@" not in prompt_c)
