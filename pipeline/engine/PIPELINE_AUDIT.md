...
| **Visual verification gap** — WebGL canvas can't be screenshotted headless, so "looks AAA" is unverified by automation | **High (open)** | **OPEN** — the vision fidelity gate is the intended fix but is unproven on a live run; this is the #1 remaining risk to "works for users" visually |
