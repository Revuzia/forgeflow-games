...
export const GAME = {
  ...
  setup(ctx) {
    ...
    for (const [cx, cy] of coins) ctx.spawn({ tag: "coin", position: [cx, cy, 0], scale: [0.5, 0.5, 1], color: __COIN__, layer: 1 });
    ctx.spawn({ tag: "goal", position: [LEN - 1, 2.0, 0], scale: [0.7, 2.4, 1], color: __GOAL__, layer: 1 });
    ctx.player = ctx.spawn({ tag: "player", position: [1.5, 1.6, 0], scale: [0.8, 0.8, 1], color: __HERO__, layer: 2 });
...
export const GAME = {
  title: __TITLE__, dim: "2d",
  setup(ctx) {
    ctx.player = ctx.spawn({ tag: "player", position: [4, 4.5, 0], scale: [0.8, 0.8, 1], color: __HERO__, layer: 2 });
    ctx.lives = __LIVES__; ctx.enemyKills = 0; ctx.fireCd = 0;
    const spots = __SPOTS__;
    ctx.enemiesTotal = spots.length;
    for (const [x, y] of spots) { const e = ctx.spawn({ tag: "enemy", position: [x, y, 0], scale: [0.9, 0.9, 1], color: __ENEMY__, layer: 1 }); e.speed = 1.25; }
    ctx.music("music", { vol: 0.5 });
...
export const GAME = {
  ...
  setup(ctx) {
    ...
    ctx.player = ctx.spawn({ tag: "player", position: [0, 0.1, 0], scale: [1, 1, 1], color: __HERO__ });
    const coins = __COINS__;
    ctx.coinsTotal = coins.length;
    for (const cx of coins) ctx.spawn({ tag: "coin", position: [cx, 0.1, 0], scale: [0.7, 0.7, 0.7], color: __COIN__ });
    ctx.music("music", { vol: 0.5 });
...
GENRE_ROUTER = {
    "platformer": platformer_js, "runner": platformer_js,
    "shmup": shooter_js, "shooter": shooter_js, "arcade": shooter_js, "bullet-hell": shooter_js,
    "tactics": collect_js, "tactics3d": collect_js, "3d-tactics": collect_js, "void-skirmish": collect_js,
    "collect": collect_js, "pickup": collect_js,
}


def emit_game_js(content):
    genre = (content.get("genre") or "").lower()
    fn = GENRE_ROUTER.get(genre, collect_js)        # default: a safe, winnable 3D collect game
    return fn(content)
...
def build(content_path, out_dir, slug="engine-game"):
    ...
    js, meta = emit_game_js(content)
    (out / "game.js").write_text(js, encoding="utf-8")
    for sub in ("src", "assets"):                    # bundle the engine runtime + the assets it fetches
        dst = out / sub
        if dst.exists():
            shutil.rmtree(dst)
