...
const STYLES = {
  flame:  { color: '#ffab3a', size: 0.62 },   // burning oil
  oil:    { color: '#c8742a', size: 0.5 },
  arcane: { color: '#c89aff', size: 0.46 },   // spire missiles
  spark:  { color: '#9adcff', size: 0.4 },    // storm
  holy:   { color: '#ffe9a0', size: 0.5 },
  nature: { color: '#8fd435', size: 0.48 },   // thorns
  runic:  { color: '#5ad8e8', size: 0.5 },    // rune traps
  smoke:  { color: '#8a8a92', size: 0.9 },
  gold:   { color: '#ffd76a', size: 0.42 },
  scrap:  { color: '#c8b090', size: 0.44 },   // construct debris
};
...
class Ambient {
  constructor(scene, kind, rngSeedless = Math.random) {
    this.kind = kind;
    const conf = {
      sandmotes:  { n: 90, color: '#e8cc8a', size: 0.28, op: 0.6 },
      rain:       { n: 260, color: '#9ab8d8', size: 0.26, op: 0.7 },
      cloudwisps: { n: 60, color: '#ffffff', size: 0.6, op: 0.4 },
      sparkles:   { n: 140, color: '#d8b8ff', size: 0.3, op: 0.85 },
  ...
  update(dt, t) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (this.kind === 'rain') {
        this.pos[k + 1] -= dt * 14;
        if (this.pos[k + 1] < 0.1) this.pos[k + 1] = 10;
      ...
      } else if (this.kind === 'sandmotes' || this.kind === 'cloudwisps') {
