...
import { CARDS, COLLECTIBLE, TOKENS, SET_STATS, cardById, REALMS } from './runtime/sim/cards.js?v=11';
import {
...
} from './runtime/sim/engine.js?v=11';
import { validateDeck, STARTER_DECKS, starterDeckErrors, DECK_SIZE } from './runtime/sim/decks.js?v=11';
import { chooseAction, runAiTurn, findLethal, evaluate, DIFFICULTIES } from './runtime/sim/ai.js?v=11';
...
{
  const { CHAPTERS, ACHIEVEMENTS, CARDBACK_INFO, allBattles } = await import('./runtime/campaign/campaign_data.js?v=11');
  const prog = await import('./runtime/campaign/progression.js?v=11');
  const { EXPANSION_IDS } = await import('./runtime/sim/cards.js?v=11');
...
if (args.has('--assets')) {
  ...
  const { CARDBACK_INFO } = await import('./runtime/campaign/campaign_data.js?v=11');
  ...
  for (const [, cid, file] of miniFiles) {
    ...
    // minis are for the marquee tiers only — legendaries + curated epics
    ok(def && (def.rarity === 'legendary' || def.rarity === 'epic'),
       `mini ${cid} is a legendary/epic card`);
    ok(def?.type === 'creature', `mini ${cid} is a creature`);
