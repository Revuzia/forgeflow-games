...
import { CARDS, COLLECTIBLE, TOKENS, SET_STATS, cardById, REALMS } from './runtime/sim/cards.js?v=9';
import {
...
} from './runtime/sim/engine.js?v=9';
import { validateDeck, STARTER_DECKS, starterDeckErrors, DECK_SIZE } from './runtime/sim/decks.js?v=9';
import { chooseAction, runAiTurn, findLethal, evaluate, DIFFICULTIES } from './runtime/sim/ai.js?v=9';
...
{
  const { CHAPTERS, ACHIEVEMENTS, CARDBACK_INFO, allBattles } = await import('./runtime/campaign/campaign_data.js?v=9');
  const prog = await import('./runtime/campaign/progression.js?v=9');
  const { EXPANSION_IDS } = await import('./runtime/sim/cards.js?v=9');
...
if (args.has('--assets')) {
  ...
  const { CARDBACK_INFO } = await import('./runtime/campaign/campaign_data.js?v=9');
