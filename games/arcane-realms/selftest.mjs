...
import { CARDS, COLLECTIBLE, TOKENS, SET_STATS, cardById, REALMS } from './runtime/sim/cards.js?v=13';
import {
...
if (args.has('--assets')) {
  ...
  const { CARDBACK_INFO } = await import('./runtime/campaign/campaign_data.js?v=13');
  ...
  // 3D hero characters — one per realm
  for (const realm of ['ember', 'tide', 'grove', 'dawn', 'grave', 'neutral']) {
    ok(existsSync(join(HERE, 'assets', 'heroes', `hero_${realm}.glb`)), `hero glb ${realm}`);
  }
}
