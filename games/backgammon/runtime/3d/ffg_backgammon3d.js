...
// warm board-room theme
const THEME = {
  sky_top: 0x3b4454, sky_bot: 0x232830,
  felt: 0x2f5138, frame: 0x5a3a24, frameRim: 0x6b4a2e,
  tri_a: 0xd8b071, tri_b: 0x8a4d2b,     // ivory-tan vs warm walnut points
  bar: 0x4a3320, tray: 0x3a2717,
  fog: 0x262b33, fogD: 0.008,
  key: 0xfff3df, keyI: 1.35, hemiSky: 0xbfd2e6, hemiGround: 0x4a3a2a, hemiI: 0.7, ambI: 0.5, exposure: 1.02,
};
const SIDE_COL = { w: 0xe9dcc0, b: 0x2a2730 };      // ivory vs charcoal checkers
...
register3d("backgammon3d", async (kernel, content) => {
  ...
  if (kernel.enableBloom) kernel.enableBloom({ strength: 0.14, radius: 0.5, threshold: 0.9 });
  ...
  const DISC_MAT = {
    w: new THREE.MeshStandardMaterial({ color: SIDE_COL.w, roughness: 0.42, metalness: 0.04, envMapIntensity: 0.8 }),
    b: new THREE.MeshStandardMaterial({ color: SIDE_COL.b, roughness: 0.4, metalness: 0.14, envMapIntensity: 0.6 }),
  };
  ...
  (function buildBoard() {
    const plinth = new THREE.Mesh(new THREE.BoxGeometry(BWH * 2 + COL * 1.4, 0.9, ZEDGE * 2 + COL * 1.0),
      new THREE.MeshStandardMaterial({ color: shade(THEME.frame, -0.25), roughness: 0.7, metalness: 0.12 }));
    ...
    const felt = new THREE.Mesh(new THREE.BoxGeometry(BWH * 2, 0.16, ZEDGE * 2),
      new THREE.MeshStandardMaterial({ color: THEME.felt, roughness: 0.95, metalness: 0, envMapIntensity: 0.1 }));
    felt.position.set(0, FELT_Y - 0.08, 0); felt.receiveShadow = true; scene.add(felt);
    // outer frame rails
    const railMat = new THREE.MeshStandardMaterial({ color: THEME.frame, roughness: 0.6, metalness: 0.16 });
    ...
    const bar = new THREE.Mesh(new THREE.BoxGeometry(BAR_W, BAR_TOP_Y + 0.4, ZEDGE * 2),
      new THREE.MeshStandardMaterial({ color: THEME.bar, roughness: 0.6, metalness: 0.18 }));
    ...
    const matA = new THREE.MeshStandardMaterial({ color: THEME.tri_a, roughness: 0.82, metalness: 0, side: THREE.DoubleSide, envMapIntensity: 0.12 });
    const matB = new THREE.MeshStandardMaterial({ color: THEME.tri_b, roughness: 0.82, metalness: 0, side: THREE.DoubleSide, envMapIntensity: 0.12 });
    for (let i = 0; i < 24; i++) {
    ...
    // bear-off trays
    const trayMat = new THREE.MeshStandardMaterial({ color: THEME.tray, roughness: 0.7, metalness: 0.14 });
  ...
  function buildDie(value, used) {
    ...
    const body = new THREE.Mesh(new THREE.BoxGeometry(DIE_SZ, DIE_SZ, DIE_SZ),
      new THREE.MeshStandardMaterial({ color: used ? 0x6b6b70 : 0xf3ede0, roughness: 0.45, metalness: 0.08, transparent: used, opacity: used ? 0.5 : 1 }));
    body.castShadow = true; g.add(body);
    const pipMat = new THREE.MeshStandardMaterial({ color: used ? 0x3a3a40 : 0x2a2730, roughness: 0.5 });
