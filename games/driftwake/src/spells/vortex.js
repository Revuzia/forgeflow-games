 ...
 * *back*. A brush with a negative depression is a perfectly ordinary brush as far
 * as the simulation is concerned — the accumulation is additive and the clamp
 * floors it at zero — so "remove snow from a ring" and "put it back" are the same
 * code path as everything else, with a sign on it.
 *
...
const HELICES = 3;
/** Spine samples per helix. See STRAND_COLS — this is the tightest curve drawn. */
const COLS = 64;
...
export class Vortex {
    ...
    update(dt) {
        ...
        // The column FOLLOWS THE PLAYER. It is *their* vortex — walking out of it
    ...
    _helices(env) {
        ...
        for (let hIdx = 0; hIdx < HELICES; hIdx++) {
            ...
            for (let c = 0; c < COLS; c++) {
                const u = c / (COLS - 1);
                // Column 0 is the TOP of the helix — the leading edge of the lift
                // — so `u` runs downward, matching every other strand.
                const h = 1 - u;
                ...
                if (c > 0) {
                    ...
                    rx = 0; ry = 1; rz = 0;      // seed frame is world UP
                }

                // Both ends taper to nothing: the top because the snow is
                // dispersing, the bottom because it is still on the ground.
                //
                // THIN. The helices are here to give the column a readable *shape*,
                // not to be the column: the mass of it is the grains, and a fat
                // ribbon turns the spell into three solid loops with some snow near
                // it.
                //
                // ANTI-VERTEBRAE RULE 1: monotonic in `u`, ONE slow modulation and
                // nothing else. Several terms keyed to world distance reach the
                // sample Nyquist and pinch the tube shut wherever their zeros line
                // up, which reads as vertebrae. Anything finer than the samples can
                // carry has to live in the relief field, which is band-limited on
                // purpose.
                const taper = bell(u * 0.92 + 0.04);
                const rad = 0.125 * taper * env
                          * (0.78 + 0.34 * Math.sin(u * 3.4 + ctx.time * 2.2 + hIdx));

                // ANTI-VERTEBRAE RULE 2: the section roll carries NO distance term.
                // A roll that advances along the spine spirals everything keyed to
                // the section angle — including the relief field — so the surface
                // comes out cut with a screw thread, which on a thin tube reads as
                // vertebrae. The Ribbon wants that spiral because it has an
                // elliptical section and the twist is the point; a round section
                // gains nothing from it but the artefact.
                water.column(
                    s, c, x, y, z, rad,
            ...
            // amount of transparency left is what lets the far side of the column
            // show through the near side, which is most of what makes it read as a
     ...
     *
     * The ring grows outward while the spell holds and retreats while it fades, so
     * the snow comes back FROM THE OUTSIDE IN — which is what settling snow does,
     * since the outermost material was lifted the least far.
     */
    _strip(dt, env) {
        ...
        const N = 9;
        // HOLDING: take snow away — depression up, essentially no berm, because
        // the mass is in the air rather than piled at the rim.
        // FADING: put it back — a NEGATIVE depression, so the sim's additive
        // accumulation fills, plus a real berm because what lands is broken snow
        // sitting proud of what it fell on, plus negative compression because
        // fresh fallen snow is loose. The fade writes are deliberately NOT scaled
        // by env: the giving-back happens at full strength even as the visual
        // fades out, which is what makes the ground end up refilled.
        ...
        for (let i = 0; i < N; i++) {
            ...
            const r = this.ring * (0.82 + rand() * 0.3);
            f.brush(
                this.x + Math.cos(a) * r,
                this.z + Math.sin(a) * r,
                0.55,
     ...
     *
     * The radius expression is character-for-character the one the helix mesh
     * uses, times a jitter. That is what makes the grains and the geometry agree.
     */
    _grains(dt, env) {
        const ctx = this.ctx;
        const sp = ctx.spray;
        ...
        const rate = 2600 * ctx.sprayScale * env;   // the highest emission rate here
        ...
        for (let k = 0; k < count; k++) {
            ...
            // (-sin, +cos) is d/dang of (cos, sin), so it follows the helix the
            // mesh is drawing whichever way round the frame is handed.
            ...
            sp.emit(
                this.x + cs * r,
                groundY + TOP * h * env + 0.06 + rand() * 0.2,
                this.z + sn * r,
                // Note the -0.6 rather than -0.5: a slight inward bias.
                vx + cs * (rand() - 0.6) * 1.2,
