...
frame for frame.
    ...
    python shoot.py --url http://localhost:8799/games/driftwake/index.html --out shots/port
...
(() => {
  ...
  window.__sfChrome = () => {
    for (const sel of ['#boot', '#hint', '#overlay', '#crosshair', '#spellbar', '#hud', '#minimap', '#floaters', '#enemybars', '#xphud', '.overlay', '#perf']) {
