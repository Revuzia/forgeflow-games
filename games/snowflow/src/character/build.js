 ...
 *
 * Port of `snowflow_demo/src/character/build.js`. Nothing here is authored in a
 * DCC tool: every surface is a lofted tube, a swept ring or a Bezier-blended
 * shell evaluated from the bind-pose skeleton, so the whole figure is a few
 * hundred lines of tables plus one smooth-normal pass.
 *
 * Three geometries come out, because three different vertex programs drive them:
 *
 *   body   two-influence blend skinning — head, cowl, torso, arms, trousers,
 *          boots, belt, scarf.
 *   cloth  driven from the simulated garment grids, sampled with Catmull-Rom in
 *          the vertex shader. Carries NO positions: `position` is (u, v, panel).
 *   fur    shell fur: the same rim ring emitted 22 (hood) / 18 (cuff) times,
 *          each pushed further along its own direction, alpha-tested to strands.
 *
 ...
 * both less code and immune to the sign errors analytic normals on a swept
 * surface invite. Closed rings share their seam vertex rather than duplicating
 * it, so the seam is smooth too. Only the fur supplies explicit normals (the
 * shell direction).
 *
 ...
 * ===========================================================================
 *
 * Every table below is transcribed VERBATIM in the reference's frame: Babylon,
 * left-handed, +x right, +y up, **+z forward**, metres, bind-pose world space.
 * `core/camera.js` fixes the project-wide convention that the port's world is
 * the reference's world **mirrored in z**, so exactly one operation converts the
 * whole file: `mirrorZ()` at the end of `finish()`, which negates z on every
 * position and every normal and reverses each triangle's winding.
 *
 * Doing it once, at the boundary, rather than by negating forty individual `z`
 * literals is deliberate. Mirroring flips the sign of every cross product, so a
 * per-literal transcription would silently reverse the loft's section frame, the
 * cowl's rim parameter and the fur band's cross-section arc — each of which is a
 * different reparameterisation with a different visible symptom. Mirroring the
 * finished arrays cannot get any of those wrong, because it never re-evaluates a
 * cross product: normals are computed in the reference frame and then carried
 * through the same mirror as the positions they belong to.
 *
 * (Winding is reversed so the mesh stays consistent with its own normals. All
 * three character materials are double-sided and the fabric fragment shader
 * turns N toward the viewer per pixel, so nothing downstream depends on it —
 * but leaving an inside-out mesh behind is how the next person loses an hour.)
 *
 * The cloth panels are the exception and are NOT built here: their mirror also
 * has to reverse the column order, for reasons set out in `cloth.js`.
 */
...
/** The cowl. 34 columns x 9 rows of quads, swept quadratic Bezier. */
const HOOD_COLS = 34;
const HOOD_ROWS = 9;
/** Skull centre, bind-pose world space (reference frame, +z forward). */
const HEAD_C = [0, 1.655, 0.005];
/** Unit already: 0.28^2 + 0.96^2 = 1. Points out of the face opening. */
const FACE_DIR = (() => {
    const v = [0, -0.28, 0.96];
...
class Builder {
    constructor() {
        ...
        this.aux = [];
        this.bi = [];       // bone indices, 4 per vertex, only 2 ever used
        this.bw = [];       // bone weights, 4 per vertex, only 2 ever used
        this.idx = [];
...
/**
 * Area-weighted smooth normals, in the REFERENCE frame (mirrored later).
 *
 * Area weighting rather than plain averaging: a long thin triangle at a cap
 * would otherwise pull the pole normal off toward its own plane. The
 * un-normalised cross product's length is twice the triangle's area, which is
 * exactly the weight wanted, so the weighting is free.
 *
 * Three's `computeVertexNormals()` is the same algorithm, but it is run here by
 * hand because it has to happen BEFORE the z mirror — afterwards the winding is
 * reversed and it would produce inward normals.
 */
 ...
 * plane is derived by CENTRAL DIFFERENCE from the neighbouring rings, so a limb
 * that bends in the bind pose still gets circular sections rather than sheared
 * ones.
 *
 * @param {Builder} B
 * @param {number[][]} rings
 * @param {number} matId
 * @param {number[]} ref reference axis the section frame is built against
 */
function loft(B, rings, matId, ref, capStart, capEnd) {
    ...
    for (let r = 0; r < n; r++) {
        ...
        // Reference frame (left-handed, +z forward); the mirror in finish()
        // takes care of the port's frame without re-deriving these.
        ...
        // UVs are metres of surface, not normalised. Every scale in the fabric
        // shader — the weave, the yarn slub — is a physical size, and normalised
        // UVs would give each body part its own thread pitch.
        ...
        if (r === 0) for (let s = 0; s < SEG; s++) first.push(row[s]);
        prevRow = row;
    }
...
/** A fan to a single centre vertex placed on the ring's own axis. */
...
function limbRings(x0, y0, z0, x1, y1, z1, r0, r1, steps, boneA, boneB, ao, from, to) {
    ...
    for (let i = 0; i <= steps; i++) {
        const t = i / steps;
        // Weight ramps from boneA to boneB across the segment's lower part, so
 ...
 *
 * Most of this is only ever seen in slivers — the robe covers the torso, the
 * mantle covers the shoulders. What is genuinely on screen is the hood
 * silhouette, the boots and the forearms, so that is where the ring counts go.
 *
...
export function buildBody() {
    ...
    for (let i = 0; i < TORSO.length; i++) {
        const y = TORSO[i][0], rx = TORSO[i][1], rz = TORSO[i][2];
    ...
    // cowl, and a half-finished face is far worse than a silhouette. The heavy
    // baked occlusion (0.22) keeps the cavity dark even when the sun swings
    // round to face it.
    const head = [];
    for (let i = 0; i <= 8; i++) {
        const a = (i / 8) * Math.PI;
    ...
    // A scarf across the lower face. It is what stops the shadowed skull
    // reading as an empty hood.
    const scarf = [
    ...
    for (let l = 0; l < 2; l++) {
        ...
        // The only loft in the file whose `ref` is not (0,0,1): the boot runs
        // along the foot's own forward axis, so it swings with the ankle roll
        // rather than being a block bolted to the shin.
        const boot = [
 ...
 * Reference frame. Reused verbatim by the fur trim so the two cannot drift.
 *
 * Evaluates to an ellipse with semi-axes 0.152 m horizontal and 0.163 m in the
 * tilted face plane, centred at (0, 1.6256, 0.1058); perimeter about 0.99 m.
 */
export function hoodRimPoint(s, out) {
    const a = s * Math.PI * 2;
    // U spans the rim horizontally, W in the face plane, both perpendicular to
    // FACE_DIR. Reference (left-handed) cross product; mirrored in finish().
...
/** Where the hood meets the shoulders. */
...
/**
 * The cowl, as a swept quadratic Bezier: each strand runs from a point on the
 * face-opening rim to a point on the ring where the hood meets the shoulders,
 * bowed outward by a control point pushed furthest over the crown. That gives a
 * genuinely deep hood with a rolled opening rather than a sphere with a hole.
 */
function buildHood(B) {
    ...
    for (let r = 0; r <= HOOD_ROWS; r++) {
        ...
        for (let c = 0; c < HOOD_COLS; c++) {
            ...
            //
            // The control direction has to be stated. It sweeps from up-and-back
            // over the crown, through sideways at the temples, to down-and-
            // forward under the chin — the same sweep the rim parameter already
            // makes, so it comes straight off `s`.
            const a = s * Math.PI * 2;
            ...
            nx /= nl; ny /= nl; nz /= nl;
            // Widest over the crown, tightest at the throat — what gives the
            // cowl its peak.
            ...
            // The inside of a cowl sees almost no sky. Ramping 0.34 -> 0.89 from
            // rim to shoulder is the single cheapest thing that makes a hood
            // read as deep.
            const ao = 0.34 + 0.55 * Math.min(1, t * 2.2);
            // UVs in metres: the rim is about a metre round, the sweep from rim
            // to shoulder about 45 cm.
 ...
 * Shell fur.
 *
 * A trim band is a partial torus around the edge it decorates: a ring of
 * cross-sections, each an arc of directions pointing away from the garment.
 * That surface is emitted once per shell, each copy pushed further along its own
 * direction, and the fragment shader alpha-tests a hashed strand field whose
 * survival threshold rises with the shell parameter — so strands taper, end at
 ...
 * bone and the cuffs ride the forearms, both rigid. Binding fur to a simulated
 * surface would need the shell direction to come out of the cloth solve — a
 * second vertex program, for very little visible gain.
 *
 * @returns {THREE.BufferGeometry}
 */
export function buildFur() {
    ...
    for (let a = 0; a < 2; a++) {
        ...
        // ring sits in the XZ plane around it and its outward is purely radial.
        for (let c = 0; c < n; c++) {
            const ang = (c / n) * Math.PI * 2;
            const rx = Math.sin(ang), rz = Math.cos(ang);
            // y = 0.900 is 3.4 cm above the wrist joint (0.866): deliberately on
            // the sleeve's tightly-pinned rows 5-6 (pinRate 9 and 5), where the
            // garment cannot visibly separate from a bone-bound band.
...
function emitFurBand(B, cols, bases, outs, r0, len, shells, bone, ao) {
    ...
    // rotated about the ring's own tangent. `A = t x o` is deliberately NOT
    // renormalised — where the tangent and the outward direction are not exactly
    // perpendicular, |A| < 1 and the arc is slightly squashed, which is what the
    // reference draws.
    for (let c = 0; c < cols; c++) {
        const cn = (c + 1) % cols;
    ...
    // Cumulative arc length around the ring, so the strand field has a uniform
    // pitch in metres regardless of how big the band is. The shader multiplies
    // this by a density in cells per metre; anything else makes hood fur and
    // cuff fur come out at different scales.
    ...
    for (let s = 0; s < shells; s++) {
        ...
        for (let c = 0; c <= cols; c++) {   // INCLUSIVE: the seam column is
            const ci = c % cols;            // duplicated so the strand field
            for (let k = 0; k <= FUR_ARC_STEPS; k++) {   // does not wrap-seam
                const o = (ci * stride + k) * 3;
                const dx = dir[o], dy = dir[o + 1], dz = dir[o + 2];
                const rad = r0 + len * t;
                // `across` uses r0, NOT rad. The V coordinate of a given (c, k)
                // is therefore identical on every shell, as is arc[c] for U —
                // which is what makes a strand a coherent vertical column
                // through the shell stack rather than a smear. Using `rad` here
                // splays the strands and the fur reads as a fuzzy halo with no
                // individual fibres.
                ...
                const vi = B.vert(
                    ...
                    t, ao,                   // aux = (shell parameter, baked ao)
                    bone, 1, 0, 0
                );
                B.normal(vi, dx, dy, dz);    // explicit: the shell direction
            }
        }
...
/**
 * Mirror the reference frame into the port's, then pack into a BufferGeometry.
 *
 * `mirrorZ` is the ONLY handedness conversion in this file. See the file header
 * for why it is one operation at the boundary rather than forty negated literals.
 */
function finish(name, B) {
    ...
    // --- the mirror ---------------------------------------------------------
    // p_port = diag(1,1,-1) * p_ref, and the same for every normal, which is the
    // correct normal for the mirrored surface. Reversing each triangle's winding
    // restores the agreement between winding and normal that mirroring breaks.
    ...
    g.setAttribute("aux", new THREE.BufferAttribute(new Float32Array(B.aux), 2));
    // Kept as FLOAT vec4s exactly as the reference does: integer attributes would
    // need Int32BufferAttribute plus flat interpolation qualifiers, for a shader
    // that just calls int() on them anyway.
    g.setAttribute("boneIdx", new THREE.BufferAttribute(new Float32Array(B.bi), 4));
    g.setAttribute("boneWt", new THREE.BufferAttribute(new Float32Array(B.bw), 4));
    g.setIndex(new THREE.BufferAttribute(idx, 1));

    // The mesh is placed entirely by the vertex shader from bone matrices, so
    // its world matrix is the identity for ever and its bounds are a lie.
    g.boundingSphere = new THREE.Sphere(new THREE.Vector3(), 1e6);
    g.name = name;
    g.userData.triangles = idx.length / 3;
    g.userData.vertices = pos.length / 3;
    return g;
}
...
/**
 * The render lattice for the simulated garments.
 *
 ...
 * render as a smooth 72x32 surface, and it means the sim cost is independent of
 * how finely the garment is tessellated.
 *
 * No z mirror is needed here: there is no z to mirror. The reversal that the
 * mirror requires lives in the panel generators (`cloth.js`), which emit their
 * columns in reversed order.
 *
 * Both `i = 0` (u = 0) and `i = renderCols` (u = 1) are emitted; the u-wrap in
 * `clothNode` makes them evaluate to exactly the same 16 taps, so the tube is
 * watertight and smooth-shaded across the seam. Only the `uv` differs, which is
 * what makes the weave run once around the tube per `weaveU` metres.
 *
 * @param {import("./cloth.js").ClothPanel[]} panels
 * @returns {THREE.BufferGeometry}
 */
export function buildClothMesh(panels) {
    const pos = [];
    const uv = [];
    const aux = [];
    const idx = [];
    ...
    g.setAttribute("aux", new THREE.BufferAttribute(new Float32Array(aux), 2));
