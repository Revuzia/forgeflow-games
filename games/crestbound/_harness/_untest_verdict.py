...
CLAIM = [
 ...
 ("heroHidden", r"(camera|lens|frame|shot|view).{0,60}(nim|hero|him|his (head|skull|scalp|helmet))|(nim|hero) (is )?(not|never) (visible|on ?screen|in frame)|inside (nim'?s|the hero'?s|his) (head|skull)|top-?down|over-?the-?head|scalp|occlude[sd]? (nim|the hero)|covers? (nim|the hero)|camera (is )?inside him|collapse[sd]? to (zero|\d)"),
 ("viewBlocked", r"(hide|hides|hid|blocking|blocks|covers|fills|eats|swallow)s? (the|its|his) ?(view|world|rest|whole|entire|forward|screen|frame|left|right|middle|doorway|loft|run|walk|road|gauntlet|catwalk)|stands? between the (camera|lens) and|in front of the (lens|camera)|you cannot see (any|the)|hidden behind"),
 ...
 ("unbroken",   r"broken[=:] ?false|still reports? broken|will not break|does not break|breaks? nothing|collider active|unobtainable"),
 ...
 ("hudMissing", r"no (breath|air|oxygen) ?(bar|meter|gauge)?|nowhere in the HUD|no air|breath bar"),
 ("surfaces",   r"reports? the default surface|every (grate|deck|plate|stair).{0,40}surface|step_metal never|surface '?normal'?"),
 ("ringOverlay", r"(wing|power) ?rings? (are )?(drawn|on screen|visible)|all ten .{0,20}rings|rings? .{0,30}before (you|the player) (has|have)"),
 ...
 ("visual",     r"render(s|ed|ing)? as|reads? as|looks? like|washed out|milky|z-?fight|banding|near-?black|flat (grey|gray|white|pale|opaque|saturated)|naked primitive|blown-?out|featureless|hard-?edged|silhouette|olive|steel blue|colou?r|texture|no gradient|no blend|seam"),
]


def claims(d):
    t = " ".join([d.get("happened") or "", d.get("should") or "", d.get("where") or ""])
...
def decide(key):
    ...
    if phase:
        ev.append("hazard phases: %d samples over %s s, kill on %.0f%% of them%s, ground %s..%s" % (
            ...
            phase.get("groundMin"), phase.get("groundMax")))
    if walk4:
    ...
    if "kill" in C and (m.get("standlong") or phase):
        if not strong:
            return weak()
        if died:
        ...
        if phase.get("n") and m.get("standlong"):
            return "FIXED", ("no death: 6 s standing still, and every one of %d hazard phases across %s s "
                             "leaves the spot survivable — %s" % (phase["n"] + 1, phase["tmax"], E)), "untest probe"
    ...
    if walk4 and ("immobile" in C or cls == "stairs"):
        if not strong:
            return weak()
    ...
    if "heroHidden" in C and cam:
        if not strong:
            return weak()
        bad = (cam.get("occludedYaws", 0) > 0 or cam.get("hiddenYaws", 0) > 0
               or (cam.get("minDist") is not None and cam["minDist"] < 1.55))
        if bad:
        ...
        return "FIXED", ("the hero is framed at >= minDist and nothing stands between the lens and him at "
                         "any of %d yaws — %s" % (cam.get("yaws", 0), E)), "untest probe"
    ...
    if "viewBlocked" in C and scr:
        ...
        # a thing that HIDES THE VIEW stands near the lens; a terrain chunk behind
        # the hero can cover the same screen box and hide nothing.
        big = [b for b in scr["blockers"]
               if b["area"] >= 0.18 and (b.get("dist") is None or b["dist"] <= 0.75 * hd)]
        if big:
    ...
    if pnd and ("unbroken" in C or re.search(r"breakable|urn|grate|hay wall|coral|plug", (d.get("where") or ""), re.I)):
        if not strong:
            return weak()
    ...
    return None, ("the battery ran at the tester's own coordinate and nothing in it contradicts or confirms "
                  "his words; the claim needs a beat this battery does not stage — " + E), "untest probe (inconclusive)"
