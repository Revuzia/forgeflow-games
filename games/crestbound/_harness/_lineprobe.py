...
with sync_playwright() as pw:
    ...
    pg.evaluate("""async () => {
      ...
      G.player.__test.teleport(new THREE.Vector3(4.20,18.4,-22.00));
      G.player.__test.setVel(new THREE.Vector3(0,0,0));
      G.cam.dist=8; G.cam.recenter&&G.cam.recenter();
      for(let i=0;i<60;i++) await new Promise(r=>requestAnimationFrame(r));
    }""")
    pg.wait_for_timeout(500)
    pg.screenshot(path=os.path.join(ROOT,"_shots","_probe_lines_all.png"))
    print(json.dumps(pg.evaluate("""() => {
      ...
      if(g){ g.visible=false; out.hidGrass=true; }
      out.heroY=A.game.player.pos.y;
      const hf=C.broadphase&&C.broadphase.heightfields&&C.broadphase.heightfields[0];
      out.hfAt = hf? hf.heightAt(4.2,-22.0) : null;
      return out; }""")))
    pg.wait_for_timeout(700)
    pg.screenshot(path=os.path.join(ROOT,"_shots","_probe_lines_nograss.png"))
    b.close()
