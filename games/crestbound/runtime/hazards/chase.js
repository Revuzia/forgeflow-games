...
// CRESTBOUND — the run-or-die front.
//
...
//   mat:'wall' — an advancing wall of energy on a hex lattice, dragged by support pylons
//
...
// Hex lattice + travelling scanlines + edge fresnel. Reads as a containment field, not fog.
const WALL_FRAG = `
uniform vec3 uColor;
uniform vec3 uHot;
uniform float uTime;
...
class ChaseHazard extends Hazard {
  constructor(def, ctx) {
    ...
    // default covers a normal course corridor without the author having to think about it.
    ...
    this.hud = {
      type: 'chase',
      label: this.style === 'lava' ? 'LAVA FRONT' : this.style === 'void' ? 'COLLAPSE' : 'ENERGY WALL',
      value: 0, distance: 0,
    };
    this.reset(0);
  }

  /** Front position along `axis` at course time `t`. Pure. */
  frontAt(t) {
  ...
  _buildFront(q) {
    ...
    // readability element, because it is what you see in your peripheral vision while running.
    const rimParts = [];
  ...
  _buildDebris(q) {
    ...
    const geo = mergeAll([shard]);
    const matKey = this.style === 'wall' ? 'metal' : 'obsidian';
  ...
  _buildKill() {
    // Everything at or behind the front dies. The box is deep enough that no geometry hides
    // in it and no gap opens up between two frames.
  ...
  _buildAmbience(q) {
    ...
    // A molten front is the loudest thing in the course; give it the looping bed, not just
    // the periodic slam, so you can hear exactly how close it is without looking back.
  ...
  update(t, dt, player) {
    ...
    this._updateWarning(t, dt, front, armed, done, player);
  }

  _updateWarning(t, dt, front, armed, done, player) {
    ...
    // vignette (setDamage) is deliberately never hijacked; setDanger / setEdgeTint are
    // optional hooks a Post build may not ship, so they stay guarded.
    if (this.style === 'lava') {
...
/**
 * The run-or-die front.
 * `{kind:'chase', axis:'x'|'y'|'z', from, to, speed, delay, mat:'lava'|'void'|'wall',
 *   p?:[lateral centre], s?:[cross-section], w?, h?, color?}`
 *
 * `from`/`to` are POSITIONS ON `axis` in metres (not a length); `speed` is m/s; `delay` is
 * SECONDS of course clock before it starts. Pure in `t`. Exposes `frontAt(t)`, `warn01` and
 * `hud` for the HUD and the audio mix.
 */
