...
register3d("navalfree", async function (kernel, content) {
  ...
  let myActions = [];        // this turn's local actions (batch = authoritative record + dropped-packet fallback)
  // Record a local action: keep it for the end-of-turn batch AND stream it LIVE ("act") so the
  // opponent watches my turn unfold one ship at a time (see enqueueRemoteAct on the receiving side),
  // instead of only seeing the whole turn once I've moved every ship.
  function netRecord(a) { myActions.push(a); if (online) { try { online.sendRaw("act", a); } catch (e) {} } }
  ...
  function tryMove(s, simX, simY) {
    ...
    if (netMode) netRecord({ k: "m", id: s.id, x: r.x, y: r.y }); // relay the RESOLVED pose (streamed live)
  ...
  function tryMovePath(s, pathSim) {
    ...
    if (!r.ok) { setHUD(reasonText(r.reason)); return; }
    if (netMode) netRecord({ k: "mp", id: s.id, path: r.path }); // relay the RESOLVED path (streamed live)
  ...
  function tryFire(shooter, targetShip) {
    ...
    if (netMode) netRecord({ k: "f", id: shooter.id, tid: targetShip.id, res: { result: r.result, dmg: r.dmg, crit: r.crit, d20: r.d20, toHit: r.toHit, sunk: r.sunk, victim: r.target, friendlyFire: r.friendlyFire, redirected: r.redirected } });
  ...
  // Apply + animate ONE opponent action, mirrored to my board. Returns a Promise that resolves when
  // its animation finishes. Used both for LIVE-streamed actions (shown the instant the opponent makes
  // them) and as the end-of-turn fallback for any action whose live "act" packet was dropped.
  async function applyOneRemoteAct(a) {
    if (!a || sim.ended) return;
    if (a.k === "m") {
  ...
  // LIVE opponent playback. Each action the opponent makes is streamed to us the moment they make it
  // (msg "act"), so we watch their turn unfold ONE SHIP AT A TIME — exactly like the AI in
  // single-player — instead of waiting for their whole turn to arrive as a batch. We serialize on
  // _remoteQ so animations never overlap, and count them so the end-of-turn batch skips what we've
  // already shown (re-playing only any action whose packet was dropped).
  let _liveApplied = 0, _remoteQ = Promise.resolve();
  function enqueueRemoteAct(a) {
    busy = true;                       // lock my input while the opponent's turn plays out live
    _liveApplied++;
    _remoteQ = _remoteQ
      .then(() => applyOneRemoteAct(a))
      .catch((e) => { try { console.warn("[tide] live act failed", e); } catch (_) {} });
    return _remoteQ;
  }
  async function applyRemoteBatch(batch) {
    busy = true;
    await _remoteQ;                    // let any still-animating live actions finish first
    const acts = batch || [];
    for (let i = _liveApplied; i < acts.length; i++) {   // normally none — fills gaps from any dropped "act" packet
      if (sim.ended) break;
      await applyOneRemoteAct(acts[i]);
    }
    _liveApplied = 0; _remoteQ = Promise.resolve();
    if (sim.ended) { busy = false; finishMatch(); return; }
    sim.endTurn(); // -> my turn
    busy = false;
    setHUD(`<span style="color:#37e0c0">Your move.</span>`);
  ...
  function buildOnlineIface() {
    return {
      ...
      onPeerMessage: (t, d) => { if (t === "act") { enqueueRemoteAct(d); return; } if (t === "hello" && d) { oppProfile = d.id ? { id: d.id, username: d.name || "Opponent" } : null; if (d.nonce && !matchNonce) matchNonce = d.nonce; } },
      startGame: (isHost) => {
        netMode = true; netFirstSide = isHost ? "player" : "enemy"; myActions = []; _liveApplied = 0; _remoteQ = Promise.resolve();
  ...
  const controller = {
    ...
    __test: {
      ...
      selectShip: (id) => { const s = sim.shipById(id); selectedId = s ? s.id : null; if (s) showSelectionGizmos(s); setHUD(); return !!s; },   // 2-client e2e: drive a SPECIFIC ship
      liveApplied: () => _liveApplied,   // 2-client e2e: how many opponent actions we've applied LIVE this turn
