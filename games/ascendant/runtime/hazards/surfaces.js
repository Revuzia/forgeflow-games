...
// ASCENDANT — surface hazards: ice, conveyor, jumppad, speedpad, wind, sticky.
//
// These are the "feel" objects. Each one owns a solid Collider whose `surface` + `props` are read
// by runtime/player/collide.js, and each one makes that physics legible BEFORE the player commits:
// ice looks slippery, a conveyor's treads really travel at `power`, a jump pad draws where it will
// send you, a speed pad's chevrons flow in the boost direction, wind is drawn by what it carries.
//
// SURFACE PROPS CONTRACT (what the player controller should read from `collider.props`):
//   surface 'ice'      props {}                              — TUNE.iceFriction / TUNE.iceAccel
//   surface 'conveyor' props {dir:Vec3, power:number}         — m/s along dir, clamp TUNE.conveyorMax
//   surface 'bounce'   props {power:number, dir:Vec3}         — power = TARGET APEX IN METRES
//   surface 'speed'    props {dir:Vec3, power:number}         — m/s boost along dir
//   surface 'sticky'   props {power:number}                   — speed MULTIPLIER (0.5 = half speed)
//   every surface also carries props.stepSfx / props.stepRate for the footstep voice.
//
// TOUCH HOOK: when a surface effect fires, the player should call
//   `collider.ref.onTouch({type:'bounce'|'speed'|'step'|'enter', point, vel, t})`
// Pads also self-detect from `ctx.player` when that hook is not wired, so the art never goes dead.
...
import {
  ...
  bevelBox, mergeAll, makeCollider, hazSfx, hazBurst, hazRandom,
  qualityOf, sparkTexture, glowTexture, roundedRectShape, resolvePlayer, resolvePost,
...
/** Euler from a stage `rot` triple (radians). */
...
class IceHazard extends Hazard {
  constructor(def, ctx) {
    ...
    // Frost crust: a thin ring of rime where the slab meets the world, so the edge reads at speed.
    const crustParts = [];
  ...
  onTouch(info) {
    if (info && info.type === 'step') {
      hazSfx(this.ctx, 'step_ice', { gain: 0.7, pos: info.point || this.center });
    }
  }
}
...
class ConveyorHazard extends Hazard {
  constructor(def, ctx) {
    ...
    this.collider = makeCollider({
      ...
      surface: 'conveyor',
      ref: this,
      props: { dir: this.fwd.clone(), power: this.power, stepSfx: 'step_metal', stepRate: 1.0 },
    });
    this.colliders.push(this.collider);
    this.reset(0);
  }

  /** Local belt-space (fwd, nrm) point + frame for an arclength `s`. Allocation free. */
  ...
  _buildFrame() {
    // Everything is authored in belt-local space (X = side, Y = nrm, Z = fwd) then rotated once.
    const parts = [];
...
class JumpPadHazard extends Hazard {
  constructor(def, ctx) {
    ...
    this.power = clamp(num(def.power, 6), 0.4, 40);          // TARGET APEX in metres
    ...
    // or the stage's own +X run direction.
    ...
    this.collider = makeCollider({
      ...
      surface: 'bounce',
      ref: this,
      props: { power: this.power, dir: this.dir.clone(), stepSfx: 'step_rubber', stepRate: 0.9 },
    });
    this.colliders.push(this.collider);
    this.reset(0);
  }
  ...
  _buildPad() {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const foot = slab(padR * 0.26, h * 0.3, padR * 0.26, Math.cos(a) * padR * 0.82, -h * 0.5 + h * 0.15, Math.sin(a) * padR * 0.82, 0.015);
      baseParts.push(foot);
    }
  ...
  /**
   * Ghost markers along the launch trajectory. Uses ASCENDANT's asymmetric gravity so the drawn
   * arc is the arc the player actually flies: rise under gravRise to `power` metres, fall under
   * gravFall. Subtle by design — a readability aid, not a HUD.
   */
  _writeArc(hSpeed) {
    const v0 = Math.sqrt(2 * TUNE.gravRise * this.power);
  ...
  update(t, dt) {
    ...
    // Self-detect a launch when the player module has not wired onTouch. Cheap and debounced.
    // Stage.setPlayer() exists precisely so pad triggers can track the player; nothing in the
    // engine calls hazard.onTouch, so self-detection is the primary path, not a fallback.
    ...
    if (pl && pl.pos && pl.vel) {
      ...
      // Track the real horizontal speed so the ghost arc predicts the player's actual launch.
      // Only redraw on a meaningful change: 11 instance matrices is cheap, but not free.
      const hs = Math.hypot(pl.vel.x, pl.vel.z);
      if (Math.abs(hs - this._arcSpeed) > 0.45) this._writeArc(clamp(hs, 0, TUNE.speedAirCap));
    }
...
/** Springy launcher. `power` is the TARGET APEX IN METRES; the ghost arc draws where you land. */
...
class SpeedPadHazard extends Hazard {
  constructor(def, ctx) {
    ...
    this.collider = makeCollider({
      ...
      surface: 'speed',
      ref: this,
      props: { dir: this.flat.clone(), power: this.power, stepSfx: 'step_metal', stepRate: 1.15 },
    });
    this.colliders.push(this.collider);
    this.reset(0);
  }
  ...
  _trigger(point) {
    ...
    // No dedicated whoosh voice exists; the bounce transient pitched right up reads as one.
    hazSfx(this.ctx, 'bounce', { gain: 0.62, rate: 1.9, pos: point || _v });
    hazBurst(this.ctx, 'spark', point || _v, {
      count: 14, speed: 7, color: this.accent.getHex(), dir: this.flat,
    });
    // Motion-line hook for the FX layer; optional by design, guarded so it can simply not exist.
...
class WindHazard extends Hazard {
  constructor(def, ctx) {
    ...
    // It is placed at the nearest point of the box to the listener, so it swells as you approach
    // and sits all around you once you are inside.
    this.windLoop = new HazLoop(this.ctx, 'wind', {
      gain: clamp(0.35 + Math.abs(this.power) * 0.018, 0.3, 0.95),
      ref: clamp(extentAlong(this.size, this.dir) * 0.35, 5, 18),
    ...
    const hz = this;
    /**
     * Non-solid influence volume. The Stage should collect `hazard.fields` and, for each, call
     * `field.apply(playerState, dt)` once per frame after movement input is gathered.
     */
    this.field = {
      type: 'wind',
      kind: 'wind',
      ...
      /**
       * Integrate onto a {pos, vel} state. Returns the 0..1 strength applied.
       * If the hazard already pushed this exact target through Player.addWind() this frame, the
       * strength is reported but NOT applied again — belt and braces against double force.
       */
      apply(state, dt) {
  ...
  /** Convenience for anything that wants the vector without touching `fields`. */
  ...
  _buildEdge() {
    // A whisper of a frame so the volume boundary is discoverable without being a solid-looking box.
    const w = this.size.x, h = this.size.y, d = this.size.z;
  ...
  update(t, dt) {
    ...
    // The push itself. Player.addWind() is a per-frame acceleration that the controller clears
    // every update, and Stage.update() runs BEFORE Player.update(), so a value written here is
    // consumed in the same frame. This is the primary path: nothing in the engine walks `fields`.
    const pl = resolvePlayer(this.ctx);
    let inside = 0;
    ...
    // Screen-edge smear hook — optional; no Post build ships one today, so it stays guarded.
 ...
 * The Stage must collect `hazard.fields` and call `field.apply(playerState, dt)` each frame —
 * wind is deliberately NOT a collider, so it never becomes an accidental wall.
 */
...
class StickyHazard extends Hazard {
  constructor(def, ctx) {
    super(def, ctx, 'sticky');
    const q = qualityOf(ctx);

    this.center = v3(def.p, 0, 0, 0);
    this.size = sizeVec(def.s, 5, 0.45, 5);
    this.slow = clamp(num(def.power, 0.5), 0.15, 0.95);   // speed MULTIPLIER
    this.quat = new THREE.Quaternion().setFromEuler(eulerOf(def.rot));
    this.top = this.center.y + this.size.y * 0.5;
    ...
    // --- tar body ------------------------------------------------------------------------------
    const body = bevelBox(w, h, d, Math.min(0.06, h * 0.3), 2.1);
    this.body = new THREE.Mesh(body, hazMat(ctx, 'rubber'));
    this.body.castShadow = true;
    this.body.receiveShadow = true;
    this.body.position.copy(this.center);
    this.body.quaternion.copy(this.quat);
    ...
    // --- wet sheen: a slow-scrolling additive film that sells "this will hold you" ---------------
    const sheenGeo = new THREE.PlaneGeometry(w * 0.98, d * 0.98, 1, 1);
    sheenGeo.rotateX(-Math.PI * 0.5);
    this.sheenTex = glowTexture(1.1);
    this.sheenMat = new THREE.MeshBasicMaterial({
      map: this.sheenTex, color: 0x6d7f5a, transparent: true, opacity: 0.22,
      blending: THREE.AdditiveBlending, depthWrite: false, toneMapped: false, fog: false,
    });
    this.own(this.sheenMat);
    this.sheen = new THREE.Mesh(sheenGeo, this.sheenMat);
    this.sheen.renderOrder = 4;
    this.sheen.position.copy(this.center);
    this.sheen.quaternion.copy(this.quat);
    this.sheen.translateY(h * 0.5 + 0.012);
    this.add(this.sheen);
    this.sheenLocalY = h * 0.5 + 0.012;
    this.sheenDrift = new THREE.Vector3(w * 0.18, 0, d * 0.18);

    // --- blisters: slow bubbling domes, pure in t ------------------------------------------------
    const n = clamp(Math.round(w * d * 0.35 * clamp(q.decor, 0.25, 1)), 4, 34);
    const domeGeo = new THREE.SphereGeometry(0.5, 10, 6, 0, Math.PI * 2, 0, Math.PI * 0.5);
    this.blisters = new THREE.InstancedMesh(domeGeo, hazMat(ctx, 'rubber'), n);
    this.blisters.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
    this.blisters.castShadow = false;
    this.blisters.receiveShadow = true;
    this.blisters.frustumCulled = false;
    this.add(this.blisters);

    const rnd = hazRandom(def, 61);
    this.blisterData = [];
    for (let i = 0; i < n; i++) {
      this.blisterData.push({
        x: (rnd() - 0.5) * w * 0.88,
        z: (rnd() - 0.5) * d * 0.88,
        r: lerp(0.10, 0.34, rnd()),
        period: lerp(3.4, 8.5, rnd()),
        phase: rnd() * 10,
      });
    }
    ...
    this.collider = makeCollider({
      ...
      surface: 'sticky',
      ref: this,
      props: { power: this.slow, drag: 1 - this.slow, stepSfx: 'step_rubber', stepRate: 0.55, squelch: 1 },
    });
    this.colliders.push(this.collider);
    this.reset(0);
  }
  ...
  update(t) {
    this.time = t;
    for (let i = 0; i < this.blisterData.length; i++) {
      const b = this.blisterData[i];
      const k = ((t + b.phase) / b.period) % 1;
      const swell = Math.sin(k * Math.PI);
      const r = b.r * (0.25 + swell * 0.95);
      _v.set(b.x, this.size.y * 0.5 - 0.01, b.z).applyQuaternion(this.quat).add(this.center);
      _s.set(r * 2, r * 1.25, r * 2);
      _m.compose(_v, this.quat, _s);
      this.blisters.setMatrixAt(i, _m);
    }
    this.blisters.instanceMatrix.needsUpdate = true;

    // The wet highlight drifts on a slow Lissajous — a shared texture must never be scrolled, so
    // the mesh moves instead.
    _v.set(
      Math.sin(t * 0.21) * this.sheenDrift.x,
      this.sheenLocalY,
      Math.sin(t * 0.17 + 1.9) * this.sheenDrift.z,
    ).applyQuaternion(this.quat).add(this.center);
    this.sheen.position.copy(_v);
    this.sheenMat.opacity = 0.16 + 0.12 * (0.5 + 0.5 * Math.sin(t * 0.8));
    this.collider.active = this.enabled;
  }

  onTouch(info) {
    // A squelch: the soft rubber step pitched down, with a little speed jitter so it never
    // machine-guns into a single repeated click.
    const rate = 0.46 + (info && info.vel ? clamp(Math.hypot(info.vel.x, info.vel.z) * 0.012, 0, 0.12) : 0);
    hazSfx(this.ctx, 'step_rubber', { gain: 0.85, rate, pos: (info && info.point) || this.center });
    if (info && info.point) {
      hazBurst(this.ctx, 'dust', info.point, { count: 4, speed: 0.9, color: 0x6d7f5a });
    }
  }
}

/** Tar / moss surface. Halves the player's speed via `props.power` (a speed multiplier). */
export function sticky(def, ctx) { return new StickyHazard(def, ctx); }
