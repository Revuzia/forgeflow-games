...
// ASCENDANT — molten hazards: `lava` (a lethal pool) and `risinglava` (the relentless floor).
//
// The molten surface is a subdivided plane driven by a vertex-displacement wave patched onto the
// shared Mats 'lava' PBR material (scrolling emissive crust cracks in the fragment stage). The
// patched material is built ONCE PER THEME and shared by every pool in the stage; per-pool values
// (amplitude, footprint, glow) ride in through an allocation-free onBeforeRender hook.
//
// Determinism: surface height, wave phase, bubble state and light flicker are all pure functions of
// the stage clock. Only one-shot effects (bubble pops, proximity rumble) are event driven, and
// reset(t) re-seeds their edge counters without replaying them.
...
export function lavaSurfaceMaterial(ctx) {
  ...
  const uniforms = {
    ...
    uLavaGlow: { value: 1 },
    uLavaHot: { value: new THREE.Color(pal.killGlow || 0xff8a2b) },
  ...
  mat.customProgramCacheKey = () => 'ascendant-lava-surface';
...
/**
 * Bind per-mesh molten parameters. The shared material's uniforms are refreshed from the mesh's
 * userData immediately before it draws — no allocation, no per-pool material clone.
 */
...
/* ======================================================================================
   ...
   molten hazard in a stage has to share it. Each one reports its own contribution here and the
   loudest wins; the composed value is pushed only when it actually moves, and a disposed hazard
   drops out cleanly instead of leaving the screen shimmering. */
...
class LavaHazard extends Hazard {
  constructor(def, ctx, rising) {
    ...
    this._prevSurfaceY = this._surfaceY;
    this._rumbleAcc = 0;
  ...
  _buildSurface(q) {
    ...
    // Molten body seen edge-on: a shallow skirt of obsidian crust just under the waterline so the
    // pool never reads as a floating plane.
    const skirtParts = [];
    const t = 0.32;
    const dy = -0.55;
  ...
  _buildRim() {
    // A bright rim strip inset just inside the pool border — the "it is hot where it meets the
    // rock" read. Additive and unlit so bloom picks it up cleanly.
    const parts = [];
   ...
   *
   * Ten pools in foundry-1 used to mean ten point lights, all visible at once,
   * every one of them evaluated per fragment on every material in the stage —
   * measured at 26 live lights and 13 fps on Intel UHD. What a pool actually
   * needs on screen is its own emissive crust (the shared molten material
   * already carries that), a glow card for the bounce, and a share of ONE
   * orange light that the stage points at whichever pool you are standing
   * nearest. So we register a light SITE with the stage budget and drive it;
   * the stage owns the single real light and never adds one to a live scene.
   *
   * Without a stage to register with (a bare hazard harness), the glow card and
   * the emissive surface still carry the look — there is no fallback light.
   */
  _buildLight() {
    const reach = Math.max(this.size.x, this.size.z) * 0.75 + 6;
    this.baseLightIntensity = clamp(reach * 0.42, 3, 26);
    this.lightSite = null;
    const stage = this.ctx && this.ctx.stage;
    if (stage && typeof stage.addLightSite === 'function') {
      this.lightSite = stage.addLightSite({
  ...
  _buildAmbience(q) {
    ...
    // synthesised looping bed; setPos() gives it distance falloff and stereo placement for free.
  ...
  // ---- public API -------------------------------------------------------------------------
  /** Surface height at stage time `t` — pure. Exposed so the HUD can draw a danger meter. */
  heightAt(t) {
  ...
  update(t, dt) {
    ...
    // light: deterministic multi-sine flicker, driving the stage's shared site
    ...
    this._updateAudioAndHud(t, dt, y);
  }
  ...
  _updateAudioAndHud(t, dt, y) {
    const player = resolvePlayer(this.ctx);
...
/**
 * A lethal molten pool. `{kind:'lava', p:[centre], s:[full size], rising?:{from,to,speed,delay}}`
 * A def carrying `rising` is routed to `risinglava` so stage authors only need one kind.
 */
...
/**
 * The relentless rising floor. Surface y = from + clamp((t - delay) * speed, 0, to - from).
