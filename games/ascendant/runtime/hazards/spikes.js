...
// ASCENDANT — spike beds.
//
// Three modes, one hazard class:
//   'static'  (default) — a permanent bed. The bevelled base plate is a SOLID, standable collider;
//                         only the volume above the plate kills. Land on the rim, live.
//   'retract'           — spikes rise and sink on a cycle with a telegraph: the plate shudders,
//                         dust puffs, and a low rumble plays a beat before the points clear the deck.
//   'wall'              — the whole assembly is rotated so the spikes point along `dir`, for
//                         lethal vertical faces beside a ledge run.
//
// Everything is instanced: one draw call for the spikes, one for the hot tips, one for the plate.
// Position is a pure function of the stage clock; reset(t) reproduces update(t) exactly.
...
/**
 * One spike: a tapered four-sided pyramid with a bevelled collar at the base and a broken,
 * slightly off-axis tip. Built once and instanced.
 */
...
/** Base plate: chamfered deck + a raised perimeter kerb + drainage slots. */
...
class SpikeHazard extends Hazard {
  constructor(def, ctx) {
    ...
    // Local extents: x/z span the bed, y is the spike height (measured along `dir`).
    if (this.mode === 'wall') {
      // A wall def gives world-space extents; convert to bed-local by taking the two largest
      // components perpendicular to `dir` as the face, and the smallest as the spike length.
      const ax = Math.abs(this.dir.x), ay = Math.abs(this.dir.y), az = Math.abs(this.dir.z);
      // Bed-local axes after setFromUnitVectors(+Y, dir): for a +/-X facing wall local X maps to
      // world Y and local Z to world Z; for a +/-Z facing wall local X maps to world X and local Z
      // to world Y. Picking the right pair keeps the bed the size the author asked for.
  ...
  _buildPlate() {
    // The plate sits with its underside on the bed origin, spanning [-len/2 .. -len/2 + plateH]
    // along the spike axis, so a static bed's deck is flush with the def's lower face.
  ...
  _buildColliders() {
    // Solid base plate — genuinely standable, which is what makes a spike bed a design element
    // rather than a coin-flip. Half extents are expressed in world space via the bed quaternion.
  ...
  /** Extension 0..1 at stage time `t` — pure. 0 = fully sunk, 1 = fully deployed. */
  extensionAt(t) {
  ...
  update(t, dt) {
    ...
    setColliderBox(this.plateCollider, _v, _v2);
    ...
    updateKillBox(this.kill, _v, this.killHalfLocal);
 ...
 *
 * The base plate is solid and standable; only the volume above it kills. In `retract` mode the
 * kill volume only arms once extension passes 34%, so the telegraph is always survivable.
 */
