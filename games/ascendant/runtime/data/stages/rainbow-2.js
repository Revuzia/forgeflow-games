 ...
 *   53 distinct platform footprints, gap coefficient of variation 0.69, never
 *   two identical obstacles in a row (maxRepeat 1), longest run without a
 *   height change over 0.75 m = 29.7 m. The sunken RED disc, the raised
 *   ORANGE gate landing and the dropped YELLOW pad deck are the authored
 *   resets that keep it under the 40 m warning line. Two warnings, both
 *   intended and quoted here with total fidelity: the catwalk's jump back up
 *   to D1's deck is roof-deleted (the risers are the way home — that is the
 *   coin's cost), and the cp1->cp2 sight-line arc grazes crusher C3 for 16%
 *   (a timed gate — that is what a crusher is).
 *
 * REACH BUDGET (CONTRACT §0 safe limits): hardest jumps are 3.4 m at +0.9 (the
 *   BLUE middle-window crossing — the house number), 4.0 m flat (YELLOW pad
 *   gap — pad optional, 4.0 < safe 4.4) and a -2.5 m walk-off drop over 1.2 m
 *   (corridor exit; temple-3 spends the same -2.5). NO sprint is required.
 *
 * HEIGHT LADDER: 1.0 (threshold) -> 0.2 (bloom disc, sunken) -> 1.2 (crusher
 *   bed) -> 1.9 (R2 court) -> 2.4 (belts) -> 3.0 (gate landing) -> 2.2 (pad
 *   dip) -> 3.4 (gate-A court) -> 4.2 (gate-B court) -> 2.6 (GREEN field) ->
 *   3.4..5.6 (ice arc) -> 8.8 (gallery) -> 9.7 (window landing) -> 9.2..9.6
 *   (INDIGO) -> 9.4 (corridor) -> 6.9 (bridge west) -> 8.6 (apex) -> 7.8 (out).
 *
...
export default {
  ...
  objects: [
    ...
    { kind: 'platform', p: [2, 0.5, 0], s: [12, 1, 10], mat: 'stone', glow: SLATE }, // top 1.0 — CP0
    ...
    /* R1 — THE FAST RING, sunken disc. Band 0 (6.0 m/s — first of its class in
       ...
       Circle x 10.2..19.8 inside deck 9.5..20.5. The disc sits 0.8 m BELOW the
       threshold so the first read happens looking DOWN at the whole circle. */
    { kind: 'platform', p: [15, -0.3, 0], s: [11, 1, 11], mat: 'stone', glow: SLATE, stripe: true }, // top 0.2, gap 1.50, -0.8
    ...
    { kind: 'platform', p: [25.5, 0.7, 0], s: [8, 1, 10], mat: 'stone', glow: SLATE, stripe: true }, // top 1.2, gap 1.00, +1.0
    ...
    { kind: 'platform', p: [33, 1.1, 0], s: [5, 1, 7], mat: 'panel', glow: SLATE, stripe: true }, // top 1.6, gap 1.00, +0.4 — CP1
    { kind: 'platform', p: [41, 1.4, 0], s: [9, 1, 10], mat: 'stone', glow: SLATE, stripe: true }, // top 1.9, gap 1.00, +0.3 — R2 court
    ...
    /* COIN 1 — the side perch: flat 1.1 m hop out past R2's circle (z 6.1). */
    ...
    /* always possible) feeds prismgate G1's window on its dwell. A sideways belt   */
    /* in, a backward belt out, a vanish hop lane for the stubborn.                 */
    /* ============================================================================ */

    { kind: 'platform', p: [48.5, 1.5, 1.5], s: [4, 1, 4.5], mat: 'panel', glow: SLATE, stripe: true }, // top 2.0, gap 1.00, +0.1
    { kind: 'conveyor', p: [54, 1.9, 0], s: [7, 1, 4.6], dir: [0, 0, 1], power: 3.0, mat: 'conveyor' }, // top 2.4 — shoves SIDEWAYS (foundry-1 quote)
    { kind: 'platform', p: [61, 1.9, 0.6], s: [6, 1, 8], mat: 'stone', glow: SLATE }, // top 2.4, gap 0.50 — CP2, the gate's staging deck
    ...
    /* THE BELT INTO THE WINDOW. Belt top 2.4 = gate sill; power 4.5 carries you
       at the window at walking-read speed; the 2.0 m hop off the belt's end
       goes THROUGH the dwell (window 1.6 x 3.0 — the hop's arc fits; a full
    ...
    /* OUT — the backward belt (foundry-2 quote, power 3.6 against you) or the
       vanish hop lane beside it: two reads of the same 3.1 s cycle. */
    { kind: 'conveyor', p: [86.5, 1.6, -1.0], s: [6, 1, 4.2], dir: [-1, 0, 0], power: 3.6, mat: 'conveyor' }, // top 2.1, gap 1.50, -0.9 — runs AGAINST you
    { kind: 'vanish', p: [84.5, 1.9, 3.6], s: [2.6, 1, 2.8], mat: 'panel', cycle: { on: 1.9, off: 0.8, warn: 0.4, phase: 0 } }, // top 2.4 — the hop lane
    { kind: 'vanish', p: [88.5, 1.9, 4.2], s: [2.6, 1, 2.8], mat: 'panel', cycle: { on: 1.9, off: 0.8, warn: 0.4, phase: 0.5 } },
    { kind: 'platform', p: [93.5, 2.4, 0.8], s: [5, 1, 6], mat: 'stone', glow: SLATE, stripe: true }, // top 2.9, gap 0.70 — CP3
    ...
    /* Two speedpads make the approach a spend (both optional: every gap is under   */
    /* the 4.4 m run-safe line without them). Then the pair: two palindromic        */
    /* 0..6..1 gates in consecutive courts, dwell 2.0 then 1.6 — "the ladder        */
    /* tightens" as data, one dwell per gate, aperture 3.67 / 3.47 m/s.             */
    /* ============================================================================ */

    { kind: 'platform', p: [100.5, 2.9, 0], s: [7, 1, 6], mat: 'stone', glow: SLATE, stripe: true }, // top 3.4, gap 1.00, +0.5
    { kind: 'speedpad', p: [99.5, 3.55, 0], s: [3.0, 0.3, 4.0], dir: [1, 0, 0], power: 12.2 }, // flush pad — a step, never a jump
    { kind: 'platform', p: [110.5, 1.7, 0], s: [5, 1, 6], mat: 'panel', glow: SLATE, stripe: true }, // top 2.2, gap 4.00 flat/-1.2 — the pad dip (4.00 < run-safe 4.4 without it)
    ...
    { kind: 'platform', p: [147.5, 2.1, 0], s: [5, 1, 7], mat: 'stone', glow: SLATE, stripe: true }, // top 2.6, gap 1.00, -1.6 — down into the field
    ...
    { kind: 'platform', p: [158, 2.1, 0], s: [14, 1, 13], mat: 'stone', glow: SLATE }, // top 2.6, gap 1.00 — the field, CP5 at its south rail
    ...
    { kind: 'platform', p: [172.5, 2.5, -0.6], s: [11, 1, 10], mat: 'stone', glow: SLATE, stripe: true }, // top 3.0, gap 2.00, +0.4
    ...
    { kind: 'platform', p: [181.5, 2.9, 1.2], s: [4, 1, 5], mat: 'panel', glow: SLATE, stripe: true }, // top 3.4, gap 1.50, +0.4 — out of the field
    ...
    { kind: 'platform', p: [188.5, 2.9, 0], s: [6, 1, 7], mat: 'stone', glow: SLATE, stripe: true }, // top 3.4, gap 2.00 — CP6
    ...
    /* THE ICE ARC. Crosswind shoves +z across x 195..207; the glazed tiles
       drift WITH it (+z offsets each hop), then I4 cuts back against it. The
       vanish pair is the against-wind line — same 3.1 s cycle both tiles. */
    { kind: 'ice', p: [195.5, 3.3, -0.8], s: [4.6, 1, 5.0] }, // top 3.8, gap 1.70, +0.4
    { kind: 'wind', p: [201, 6.5, 0], s: [12, 6, 12], dir: [0, 0, 1], power: 10, color: FROST }, // x 195..207 — the crosswind
    { kind: 'ice', p: [201.5, 3.9, 1.2], s: [4.2, 1, 4.6] }, // top 4.4, gap 1.60, +0.6, drifting +z
    { kind: 'ice', p: [207.5, 4.5, 3.0], s: [4.0, 1, 4.4] }, // top 5.0, gap 1.90, +0.6
    { kind: 'vanish', p: [205.5, 4.1, -1.8], s: [2.6, 1, 2.8], mat: 'panel', cycle: { on: 1.9, off: 0.8, warn: 0.4, phase: 0 } }, // top 4.6 — the against-wind line
    { kind: 'vanish', p: [210.8, 4.7, -0.4], s: [2.6, 1, 2.8], mat: 'panel', cycle: { on: 1.9, off: 0.8, warn: 0.4, phase: 0.5 } }, // top 5.2
    { kind: 'ice', p: [213, 5.1, 1.4], s: [4.4, 1, 4.6] }, // top 5.6, gap 1.30, +0.6, cutting BACK
    ...
    { kind: 'platform', p: [219.5, 5.3, 0], s: [7, 1, 8], mat: 'stone', glow: SLATE, stripe: true }, // top 5.8, gap 0.80, +0.2 — shaft base
    {
      kind: 'mover',
      p: [225.5, 5.3, -3.6],
      s: [2.8, 1, 2.8],
    ...
    { kind: 'platform', p: [226, 8.3, 1.6], s: [6, 1, 6], mat: 'stone', glow: SLATE, stripe: true }, // THE GALLERY, top 8.8 — CP7
    ...
    { kind: 'ice', p: [239.5, 9.0, 0], s: [4.2, 1, 4.6] }, // top 9.5, gap 1.20, -0.2 — one last glaze
    { kind: 'wind', p: [241, 12.0, 0], s: [8, 5, 10], dir: [0, 0, -1], power: 8, color: FROST }, // counter-shear over the lip
    { kind: 'platform', p: [245.5, 9.0, -1.0], s: [5, 1, 6], mat: 'stone', glow: SLATE, stripe: true }, // top 9.5, gap 1.40
    ...
    /* N2 HIGH closes the bracket, half a phase off N1: duck-beat answers
       jump-beat across the whole terrace. */
    { kind: 'platform', p: [274.5, 8.9, 0], s: [8, 1, 9], mat: 'stone', glow: SLATE }, // top 9.4, contiguous step +0.2
    { kind: 'bloom', p: [274.5, 9.4, 0.4], rmax: 3.4, band: 5, period: 6.0, phase: 0.5, ring: 'high' },
    { kind: 'platform', p: [281.5, 9.1, -0.8], s: [4, 1, 5], mat: 'panel', glow: SLATE, stripe: true }, // top 9.6, gap 1.00, +0.2 — terrace lip
    ...
    /* CP9 SPUR — the observation deck, off-axis at z -6: outside MA's reach
       (11.66 m > 9.5 + 1.7) with the whole corridor in view. */
    { kind: 'platform', p: [287.5, 9.1, -5.8], s: [6, 1, 5], mat: 'stone', glow: SLATE, stripe: true }, // top 9.6 — CP9
    ...
    { kind: 'platform', p: [292, 9.0, -2.4], s: [3, 1, 3], mat: 'panel', glow: SLATE, stripe: true }, // entry step, top 9.5 — inside the field: the crossing starts HERE
    ...
    /* THE DOUBLE RAINBOW. Drop -2.5 off the corridor lip (temple-3 spends the
       same drop), then the twin arch: D1 west, D2 east, counter-phased on the
       6 s clock; the apex gate holds the 12 s superperiod. MB's rear reach
       passes 2.5 m OVERHEAD here — quoted clear in the header. */
    { kind: 'platform', p: [321.5, 6.4, 0], s: [4.6, 1, 6], mat: 'stone', glow: SLATE, stripe: true }, // top 6.9, gap 1.20, -2.5 walk-off
    ...
    /* The bridge west deck + D1; the arch climbs 6.9 -> 7.8 -> 8.6 (apex). */
    { kind: 'platform', p: [326.6, 6.4, 0], s: [4.6, 1, 7], mat: 'stone', glow: SLATE }, // top 6.9, gap 0.50 — D1's deck
    ...
    /* COIN 3 — the catwalk slung UNDER the arch: walk off D1's deck lip onto
       it (-1.75), out past the apex's shadow, then the two risers climb back
       to the east deck OUTSIDE D2's circle (4.6 / 5.3 m out). */
    { kind: 'platform', p: [334.5, 4.9, 0], s: [9, 0.5, 1.6], mat: 'metal', glow: SLATE }, // the catwalk, top 5.15
    { kind: 'platform', p: [341.8, 5.4, -4.6], s: [2.6, 1, 2.6], mat: 'panel', glow: SLATE, stripe: true }, // riser A, top 5.9
    { kind: 'platform', p: [344.9, 6.3, -4.2], s: [2.4, 1, 2.4], mat: 'panel', glow: SLATE, stripe: true }, // riser B, top 6.8

    /* THE FINISH COURT — violet is the finish's colour and nothing else's. */
    { kind: 'platform', p: [349.8, 7.3, 0], s: [6, 1, 9], mat: 'obsidian', glow: FINISH, stripe: true }, // top 7.8, gap 2.10 flat
