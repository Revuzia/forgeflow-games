...
class C:
    ...
    def boss(self, titan, biome, tag):
        ...
        intro = False
        got = set()
        leash_n = 0; leash_last = 0.0
        was_leashed = False; leash_min_d = 1e9
        phase_target = 1
        phase_t = time.time()
        mode_t = time.time()
        toward = True
        while time.time() - t_boss < 150:
            ...
            if T["leash"] > 0:
                was_leashed = True
                leash_min_d = min(leash_min_d, d)
            elif was_leashed:
                log("    leash released at titan-boss %.1f m (min while leashed %.1f m, titan r %.1f)" % (d, leash_min_d, T["r"]))
                was_leashed = False; leash_min_d = 1e9
            if T["leash"] > 0 and leash_n < 3 and time.time() - leash_last > 0.45:
                s.release_all()
                s.bt_call("freeze", True)
                time.sleep(0.35)
                self.shot("boss_%s_P%d_leash_cable_%d" % (tag, b["phase"], leash_n), d=round(d), leash=T["leash"]); leash_n += 1; leash_last = time.time()
                s.bt_call("freeze", False)
    ...
    def leash(self, titan="molo", biome="grideast", tag="c4ge"):
        """Stand still at range in phase 2 until the winch catches; shoot the reel and log where it lets go."""
        log("== leash %s (%s / %s)" % (tag, titan, biome))
        self.run(titan, biome, 304)
        self.keep_play()
        s = self.s
        ...
        s.cheat("boss")
        t0 = time.time()
        n = 0; was = False; dmin = 1e9; last = 0.0; rel = []
        while time.time() - t0 < 160 and len(rel) < 2:
            scr = s.screen()
            if scr != "play":
                s.release_all(); self.keep_play(); continue
            o = self.w()
            if not o or not o.get("boss"):
                time.sleep(0.2); continue
            b, T = o["boss"], o["T"]
            if b["introT"] <= 0 and b["phase"] < 2:
                s.safe_js("() => { const b = window.__H_W__().boss; if (b.hp > b.maxHp * 0.6) b.hp = b.maxHp * 0.6; }")
            d = math.hypot(b["x"] - T["x"], b["z"] - T["z"])
            # hold ~150 m: step away when the rig closes in, otherwise stand still in the paint
            if d < 125:
                s.hold(world_to_keys(T["x"] - b["x"], T["z"] - b["z"]) or {"KeyS"})
            else:
                s.release_all()
            if T["leash"] > 0:
                s.release_all()
                was = True; dmin = min(dmin, d)
                if n < 4 and time.time() - last > 0.5:
                    s.bt_call("freeze", True); time.sleep(0.35)
                    self.shot("leash_%s_%d" % (tag, n), d=round(d, 1), leash=round(T["leash"], 2), r=round(T["r"], 1))
                    s.bt_call("freeze", False)
                    n += 1; last = time.time()
            elif was:
                log("    leash released at titan-boss %.1f m (min while leashed %.1f m; titan r %.1f; reel stop = 41 + r + 15 = %.1f m)" % (d, dmin, T["r"], 56 + T["r"]))
                rel.append(round(d, 1))
                was = False; dmin = 1e9
                self.shot("leash_%s_released_%d" % (tag, len(rel)), d=round(d, 1))
            time.sleep(0.05)
        s.release_all()
        log("    releases: %s" % rel)


    def reelstop(self, titan="molo", biome="grideast", tag="c4ge"):
        """Hook the titan by hand at 100–130 m in front of the rig and watch where the reel lets go."""
        log("== reelstop %s" % tag)
        self.run(titan, biome, 304)
        self.keep_play()
        s = self.s
        ...
        time.sleep(6.0)
        s.release_all()
        for k, start in enumerate((100.0, 130.0)):
            self.keep_play()
            r = s.js("""(D) => { const W = window.__H_W__(); const b = W.boss, T = W.titan;
              b.attack = null; b.cd = 99;
              T.x = b.x + Math.sin(b.heading) * D; T.z = b.z + Math.cos(b.heading) * D; T.px = T.x; T.pz = T.z;
              const ax = b.x + Math.sin(b.heading) * 44, az = b.z + Math.cos(b.heading) * 44;
              T.leash = { t: 3, lx: ax, lz: az, strength: 12 }; b.data.leash = 1; return [b.x, b.z, T.x, T.z, !!T.leash]; }""", start)
            log("    set: %s" % (r,))
            t0 = time.time(); dmin = 1e9; shot = False; seen = 0
            while time.time() - t0 < 12:
                if s.screen() != "play":
                    self.keep_play(); continue
                o = self.w()
                if not o or not o.get("boss"):
                ...
                if T["leash"] > 0:
                    seen += 1
                    dmin = min(dmin, d)
                    if not shot and d < start - 8:
                        s.bt_call("freeze", True); time.sleep(0.35)
                        self.shot("reel_%s_%d" % (tag, k), d=round(d, 1)); s.bt_call("freeze", False); shot = True
                elif seen:
                    log("    start %.0f m -> released at %.1f m after %.2f s (reel stop 41 + r %.1f + 15 = %.1f m)" % (start, d, time.time() - t0, T["r"], 56 + T["r"]))
                    break
                time.sleep(0.03)
            log("    loop end: seen %d dmin %.1f screen %s" % (seen, dmin, s.screen()))
            time.sleep(1.0)
    ...
    try:
        for part in a.only.split(","):
            try:
                ...
                elif part == "reelstop_lw":
                    c.reelstop("voltkite", "lockwater", "c4lw")
                elif part == "reelstop":
                    c.reelstop()
                elif part == "leash":
                    c.leash()
                elif part == "leash_lw":
                    c.leash("voltkite", "lockwater", "c4lw")
                elif part == "boss_c4lw":
