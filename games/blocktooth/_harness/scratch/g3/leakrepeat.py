...
def main():
    ...
    add_common_args(ap)
    ap.add_argument("--fixseed", type=int, default=-1)
    ...
    with Session(args, "leakcheck") as s:
        ...
        for i, (t, b) in enumerate(combos):
            ok, v = s.bt_call("newRun", {"titan": t, "biome": b, "seed": args.fixseed if args.fixseed >= 0 else 100 + i, "skipSlate": True})
            if not ok:
