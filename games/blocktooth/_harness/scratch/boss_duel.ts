...
console.log(JSON.stringify({ titan, biome, seed, policy: policy ?? 'human', t: Math.round(w.t), total: `${h}/${n} (${Math.round(100 * h / Math.max(1, n))}%)`, by: out, dmgTaken: Math.round(w.titan.damageTaken) }));
