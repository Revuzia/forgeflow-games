#!/usr/bin/env python
"""[boss-threat lane copy: tracker de-duplicated per telegraph (a dead tell used to be re-counted every frame
until compaction) + follow drops tagged] BLOCKTOOTH play-critic full run — title screen → tabloid with REAL keys only (page.keyboard).
...
(() => {
  ...
  const FR = window.__FR__ = { tg: [], open: new Map(), done: new WeakSet(), bossAtk: [], lastAtk: null, minHpF: 1, dips: 0, armed: true, hurt: 0 };
  const loop = () => {
    try {
      const B = window.__BT__; const W = B && B.world;
      if (W && W.titan) {
        for (const tg of (W.telegraphs || [])) {
          if (tg.owner === 'titan' || FR.open.has(tg) || FR.done.has(tg)) continue;
          const bb = W.boss, sh = tg.shape || {};
          FR.open.set(tg, { t0: W.t, fol: !!(bb && bb.data && bb.data.followX === sh.x && bb.data.followZ === sh.z), ph: bb ? bb.phase : 0 });
        }
        for (const [tg, o] of FR.open) {
          if (!tg.alive || tg.fired && tg.active <= 0) {
            FR.done.add(tg);
            if (tg.fired) FR.tg.push({ id: tg.id, fol: o.fol, ph: o.ph, tag: tg.tag, style: tg.style, owner: tg.owner, windup: +tg.windup.toFixed(2), hit: !!tg.hitTitan, t: +o.t0.toFixed(1), dmg: Math.round(tg.dmg) });
            FR.open.delete(tg);
          }
        }
...
def main():
    ...
    by = {}
    for t in tg:
        k = "%s/%s%s" % (t["owner"], "follow:" if t.get("fol") else "", t["tag"] or t["style"])
