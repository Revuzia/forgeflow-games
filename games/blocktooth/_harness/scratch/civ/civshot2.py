...
from audits import FAC_JS, UMB_JS
url = sys.argv[1]
steps = sys.argv[2:]
errors = []
with sync_playwright() as p:
    ...
    for st in steps:
        ...
        if kind == "wait":
            ...
            try: print("fac", rest, json.dumps(pg.evaluate(FAC_JS)))
            except Exception as e: print("fac error:", e)
        elif kind == "umb":
            try: print("umb", rest, json.dumps(pg.evaluate(UMB_JS)))
            except Exception as e: print("umb error:", e)
        elif kind == "perf":
